    ...
    def rightSideView(self, root: Optional[TreeNode]) -> List[int]:
        ...
        if root:
            ...
            while temp_deque:
                ...
                if curr_level == len(helper_answer):
                    helper_answer.append([])
                helper_answer[curr_level].append(curr_node.val)

        # print(helper_answer)
        # helper_answer is the previous question's solution (i.e., level order traversal of it as a nested list, 
        # where each sublist contains the values of nodes at a particular level in the tree, from left to right.
        # to get the answer for right side view , i just take the rightmost value of the helper_answer
        answer = []
        for list in helper_answer:
